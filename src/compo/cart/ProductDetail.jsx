...
const ProductDetail = () => {
  ...
  // ✅ Load Razorpay SDK
  const loadRazorpayScript = () => {
    if (document.querySelector(`script[src="https://checkout.razorpay.com/v1/checkout.js"]`)) return Promise.resolve(true);
    return new Promise((resolve) => {
      const script = document.createElement("script");
      script.src = "https://checkout.razorpay.com/v1/checkout.js";
      script.onload = () => resolve(true);
  ...
  const initPayment = async (orderData) => {
    ...
    const options = {
      ...
      handler: async (res) => {
        try {
          if (!userEmail) return alert("Please login to continue payment");
  ...
  const handleBuyNow = async () => {
    ...
    if (!userEmail) return alert("Please login to proceed.");
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto relative">
      <div className="w-2/5 md:w-[300px] h-[300px] md:h-[400px]">
        <img src={product.src} alt={product.description} className="w-full h-full object-cover rounded-lg shadow" />
        <Link to="/allproduct">
      ...
      <p className="font-semibold text-lg my-4">This product is recommended for {product.skin} skin</p>
      <p className="mt-4 text-xl font-semibold">${product.rate}</p>

      <div className="flex gap-2">
        <button onClick={handleBuyNow} className="bg-violet-500 text-white px-6 py-1 rounded my-2 hover:bg-violet-400">
          Buy Now
        </button>

        <Link to="/carts" state={{ product }}>
          <button onClick={() => addToCart(product)} className="bg-violet-500 text-white px-6 py-1 rounded my-2 hover:bg-violet-400">
            Add to Wishlist
          </button>
        </Link>
      </div>
